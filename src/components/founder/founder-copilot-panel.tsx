...
export function FounderCopilotPanel({
  ...
  return (
    <section className="flex h-[min(560px,65vh)] min-h-[320px] flex-col overflow-hidden rounded-xl border border-border bg-surface-elevated">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground">Founder Copilot</h3>
        ...
        <div className="flex gap-2">
          ...
          {messages.length > 0 && (
            <button type="button" onClick={clearChat} className="text-muted hover:text-red-300">
      ...
      <div className="flex flex-wrap gap-2 p-3 border-b border-border">
        {suggestions.map((s) => (
          <button
            ...
            disabled={loading}
            className="rounded-full border border-border px-3 py-1 text-xs text-muted hover:border-gold/40 hover:text-foreground"
          >
            {s}
          </button>
        ))}
