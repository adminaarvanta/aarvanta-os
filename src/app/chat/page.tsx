...
export default function WebsiteChatPage() {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-black p-3 sm:items-center sm:justify-center sm:p-4">
      <div className="flex min-h-0 flex-1 w-full max-w-md flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-lg shadow-gold/5 sm:h-[32rem] sm:flex-none">
        <header className="shrink-0 border-b border-border px-4 py-4">
          <div className="flex justify-center">
            <BrandLogo variant="icon" size="header" />
          </div>
          <h1 className="mt-2 text-center text-sm font-medium text-muted">Website chat</h1>
        </header>
        ...
        >
          ...
          {messages.map((m) => (
            <div
              ...
              className={
                m.direction === "inbound"
                  ? "mr-4 rounded-xl bg-surface-muted ring-1 ring-border px-3 py-2 text-sm text-foreground sm:mr-8"
                  : "ml-4 rounded-xl bg-gold/20 ring-1 ring-gold/40 px-3 py-2 text-sm text-foreground sm:ml-8"
              }
            >
              {m.direction === "outbound" && (
                <p className="text-[10px] font-medium text-gold-bright mb-0.5">
                  {m.authorName ?? "Agent"}
                </p>
              )}
              {m.content}
        ...
        >
          {messages.length === 0 && (
            <input
              ...
              className="w-full rounded-lg border border-border bg-surface-muted px-3 py-2 text-base text-foreground sm:text-sm"
            />
          )}
          <div className="flex flex-col gap-2 sm:flex-row">
            <input
              ...
              className="min-w-0 flex-1 rounded-lg border border-border bg-surface-muted px-3 py-2.5 text-base text-foreground sm:text-sm"
            />
