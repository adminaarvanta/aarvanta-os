...
const themeInitScript = `(function(){try{var t=localStorage.getItem("aarvanta-theme");var a=t==="dark"?"dark":t==="light"?"light":t==="system"?(window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):"light";document.documentElement.classList.remove("dark","light");document.documentElement.classList.add(a);document.documentElement.style.colorScheme=a;}catch(e){document.documentElement.classList.add("light");document.documentElement.style.colorScheme="light";}})();`;

/** Set googtrans before Google Translate script boots so the first paint is already localized */
const languageInitScript = `(function(){try{var lang=localStorage.getItem("aarvanta-language")||"en";var alias={he:"iw",jv:"jw",fil:"tl"};var target=alias[lang]||lang;var expire="Thu, 01 Jan 1970 00:00:00 GMT";var host=location.hostname;var domains=["",host,"."+host];if(host.indexOf(".")!==-1){var p=host.split(".");if(p.length>2)domains.push("."+p.slice(-2).join("."));}var parts=document.cookie.split(";");for(var i=0;i<parts.length;i++){var name=parts[i].split("=")[0].trim();if(!name||name.toLowerCase().indexOf("googtrans")===-1)continue;for(var d=0;d<domains.length;d++){var dp=domains[d]?";domain="+domains[d]:"";document.cookie=name+"=;expires="+expire+";path=/"+dp;document.cookie=name+"=;Max-Age=0;path=/"+dp;}}if(location.hash&&/googtrans/i.test(location.hash)){history.replaceState(null,"",location.pathname+location.search);}if(lang&&lang!=="en"){document.cookie="googtrans=/en/"+target+";path=/;max-age=31536000;SameSite=Lax";document.documentElement.lang=lang.indexOf("zh")===0?lang:lang.split("-")[0];}else{document.documentElement.lang="en";}}catch(e){}})();`;
