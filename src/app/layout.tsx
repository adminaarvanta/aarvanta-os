...
const languageInitScript = `(function(){try{var lang=localStorage.getItem("aarvanta-language")||"en";var expire="Thu, 01 Jan 1970 00:00:00 GMT";var host=location.hostname;var domains=["",host];if(host.indexOf(".")!==-1){domains.push("."+host);var p=host.split(".");if(p.length>2)domains.push("."+p.slice(-2).join("."));}var parts=document.cookie.split(";");for(var i=0;i<parts.length;i++){var name=parts[i].split("=")[0].trim();if(!name||name.toLowerCase().indexOf("googtrans")===-1)continue;for(var d=0;d<domains.length;d++){var dp=domains[d]?";domain="+domains[d]:"";document.cookie=name+"=;expires="+expire+";path=/"+dp;}}if(location.hash&&/googtrans/i.test(location.hash)){history.replaceState(null,"",location.pathname+location.search);}if(lang&&lang!=="en"){document.cookie="googtrans=/en/"+lang+";path=/";document.documentElement.lang=lang.indexOf("zh")===0?lang:lang.split("-")[0];}else{document.documentElement.lang="en";}}catch(e){}})();`;
