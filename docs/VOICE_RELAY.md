...
TwiML probes `/voice-relay/health` before greeting. Relays older than **1.9.2** ignore `skipOpening` and will greet again if `welcomeGreeting` already said Ava’s name — so production TwiML only speaks identity when the sidecar is current.
